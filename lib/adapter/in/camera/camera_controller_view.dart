...
/// App for testing
class CameraControllerView extends StatefulWidget {
  /// Default Constructor
  const CameraControllerView({super.key});

  @override
  State<CameraControllerView> createState() => _CameraControllerViewState();
}

class _CameraControllerViewState extends State<CameraControllerView> {
  ...
  Future<void> getCameras() async {
    try {
      ...
      await window.navigator.mediaDevices!
          .getUserMedia({'video': true, 'audio': false}).then((value) {
        log("permission granted for $value");
      });
      setState(() {
        cameraAccess = true;
      });
  ...
  Widget build(BuildContext context) {
    ...
    if (!cameraAccess) {
      return Center(
        child: Text('Camera access is not granted'),
      );
    }
    if (cameras == null) {
      return Center(
        child: Text('Reading cameras'),
      );
    }
